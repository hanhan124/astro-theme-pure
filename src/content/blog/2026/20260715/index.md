...
qPCR 实验结束后仪器导出长格式 Excel，每行一个测量值。研究人员需要：pivot 成宽格式矩阵、用 2^-(ΔCt) 或 2^-(ΔΔCt) 计算相对表达量、每个基因生成带误差棒的柱状图。手动做，半小时起步。
...
没用 Tauri 默认的 NSIS/WiX，而是**自建了一个 Tauri 安装器**。`include_bytes!` 把主程序嵌入安装器二进制，安装时 UAC 提权、创建快捷方式、写注册表。加 `--uninstall` 同一个 exe 变卸载器。
...
| 安装器 | 独立 Tauri 2 应用 |
| CI/CD | GitHub Actions，Win + mac 并行构建，自动发版 + 自动更新 |
...
代码开源在 [GitHub](https://github.com/hanhan124/mynx)，MIT 协议。
